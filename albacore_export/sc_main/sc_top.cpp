...
int sc_main(int argc, char* argv[]) {
    ...
    run_instruction_step(current_time, cycle_time, "Write 0x7220 to RAM[0]", 1);
    memory_map_test(top_inst.memory_map_inst, 0, 0x7220);

    in_mem_addr.write(1); in_mem_data.write(0x8002);
    run_instruction_step(current_time, cycle_time, "Write 0x8002 to RAM[1]", 1);
    memory_map_test(top_inst.memory_map_inst, 1, 0x8002);

    in_mem_addr.write(2); in_mem_data.write(0x8112);
    run_instruction_step(current_time, cycle_time, "Write 0x8112 to RAM[2]", 1);
    memory_map_test(top_inst.memory_map_inst, 2, 0x8112);

    in_mem_addr.write(3); in_mem_data.write(0x0001);
    run_instruction_step(current_time, cycle_time, "Write 0x0001 to RAM[3]", 1);
    memory_map_test(top_inst.memory_map_inst, 3, 0x0001);

    in_mem_addr.write(4); in_mem_data.write(0x9202);
    run_instruction_step(current_time, cycle_time, "Write 0x9202 to RAM[4]", 1);
    memory_map_test(top_inst.memory_map_inst, 4, 0x9202);

    in_mem_addr.write(5); in_mem_data.write(0xF000);
    run_instruction_step(current_time, cycle_time, "Write 0xF000 to RAM[5]", 1);
    memory_map_test(top_inst.memory_map_inst, 5, 0xF000);

    in_mem_addr.write(32); in_mem_data.write(3);
    run_instruction_step(current_time, cycle_time, "Write 0x0003 to RAM[32]", 1);
    memory_map_test(top_inst.memory_map_inst, 32, 3);

    in_mem_addr.write(33); in_mem_data.write(4);
    run_instruction_step(current_time, cycle_time, "Write 0x0004 to RAM[33]", 1);
    memory_map_test(top_inst.memory_map_inst, 33, 4);

    in_mem_we.write(false);
    in_start.write(true);
    run_instruction_step(current_time, cycle_time, "Start program", 1);

    in_start.write(false);

    run_instruction_step(current_time, cycle_time, "0x7220 - ldi $2, 32", 5);
    register_test(top_inst.datapath_inst, 2, 32);

    run_instruction_step(current_time, cycle_time, "0x8002 - ld $0, $2, 0", 7);
    register_test(top_inst.datapath_inst, 0, 3);

    run_instruction_step(current_time, cycle_time, "0x8112 - ld $1, $2, 1", 7);
    register_test(top_inst.datapath_inst, 1, 4);

    run_instruction_step(current_time, cycle_time, "0x0001 - add $0, $0, $1", 5);
    register_test(top_inst.datapath_inst, 0, 7);

    run_instruction_step(current_time, cycle_time, "0x9202 - st $0, 2($2)", 6);
